  ...
  <AuthenticatedLayout>
    ...
    <template #header>
      <div class="mx-auto max-w-[100rem] sm:px-6 lg:px-8">
        ...
        >
          <div>
            <h2 class="font-bold text-xl leading-tight text-gray-800 dark:text-white drop-shadow-sm">Users Management</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage system access and roles.</p>
          </div>
    ...
    <div v-if="openForm" class="fixed inset-0 z-50 px-4 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
      <div class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/50 dark:border-gray-700/50 rounded-2xl shadow-2xl max-w-lg w-full p-8 relative animate-in fade-in zoom-in duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="handleCloseForm" class="p-2 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-400 hover:text-red-500 transition-colors">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          <div>
            ...
            <TextInput id="name" type="text" class="mt-1 block w-full bg-white/50 backdrop-blur-sm" v-model="form.name" required autofocus placeholder="John Doe" />
            <InputError class="mt-2" :message="form.errors.name" />
          </div>

          <div>
            <InputLabel for="email" value="Email Address" />
            <TextInput id="email" type="email" class="mt-1 block w-full bg-white/50 backdrop-blur-sm" v-model="form.email" required placeholder="name@company.com" />
          ...
          <div>
            ...
            <div class="relative">
                <select id="role" v-model="form.role" class="mt-1 block w-full border-gray-300 dark:border-gray-600 bg-white/50 dark:bg-gray-700/50 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm backdrop-blur-sm transition cursor-pointer py-2.5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative">
              ...
              <div class="relative mt-1">
                <TextInput id="password" :type="showPassword ? 'text' : 'password'" class="block w-full pr-10 bg-white/50 backdrop-blur-sm" v-model="form.password" :required="!isEditMode" placeholder="••••••••" />
            ...
            <div class="relative">
              ...
              <div class="relative mt-1">
                <TextInput id="password_confirmation" :type="showConfirmPassword ? 'text' : 'password'" class="block w-full pr-10 bg-white/50 backdrop-blur-sm" v-model="form.password_confirmation" :required="!isEditMode && form.password" placeholder="••••••••" />
          ...
          <p v-if="isPasswordMismatch" class="text-sm text-red-500 font-bold bg-red-50 p-2 rounded-lg animate-pulse">Passwords do not match!</p>
        ...
        >
          ...
          <div class="relative z-10 -mb-[1px]">
             <div class="w-fit px-6 h-12 bg-white/40 dark:bg-slate-900/60 backdrop-blur-md border-t border-l border-r border-white/40 dark:border-white/10 rounded-t-2xl shadow-sm relative flex items-center gap-3">
                ...
                <span class="font-bold text-gray-800 dark:text-white text-sm tracking-wide shadow-black drop-shadow-sm">All Users Data</span>
            ...
            <table class="w-full text-left dark:text-white table-auto border-collapse">
              <thead>
                <tr class="bg-white/50 dark:bg-gray-800/80 backdrop-blur-md border-b border-white/20 dark:border-white/10">
                  <th class="p-5 font-semibold text-gray-600 dark:text-gray-300 text-sm uppercase tracking-wider">No</th>
                  <th class="p-5 font-semibold text-gray-600 dark:text-gray-300 text-sm uppercase tracking-wider">Name</th>
                  <th class="p-5 font-semibold text-gray-600 dark:text-gray-300 text-sm uppercase tracking-wider">Email</th>
                  <th class="p-5 font-semibold text-gray-600 dark:text-gray-300 text-sm uppercase tracking-wider">Role</th>
                  <th class="p-5 text-center font-semibold text-gray-600 dark:text-gray-300 text-sm uppercase tracking-wider">Action</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-white/20 dark:divide-white/5">
                <tr v-for="(user, idx) in users.data" :key="user.id" class="hover:bg-white/30 dark:hover:bg-white/5 transition duration-200">
                  <td class="p-5 align-middle text-sm text-gray-500 dark:text-gray-400">{{ (users.current_page - 1) * users.per_page + idx + 1 }}</td>
                  
                  <td class="p-5 align-middle">
                      <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-indigo-100/50 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-bold text-xs border border-indigo-200/50">
                            {{ user.name.charAt(0).toUpperCase() }}
                        </div>
                        <span class="font-bold text-gray-800 dark:text-gray-200">{{ user.name }}</span>
                      </div>
                  </td>
                  <td class="p-5 align-middle text-sm text-gray-600 dark:text-gray-300 font-mono">{{ user.email }}</td>
                  <td class="p-5 align-middle">
                     <span :class="{
                        'bg-purple-100/50 text-purple-700 border-purple-200/50': user.role === 'pm',
                        'bg-blue-100/50 text-blue-700 border-blue-200/50': user.role === 'pg',
                        'bg-pink-100/50 text-pink-700 border-pink-200/50': user.role === 'ds',
                        'bg-orange-100/50 text-orange-700 border-orange-200/50': user.role === 'co',
                        'bg-gray-100/50 text-gray-700 border-gray-200/50': user.role === 'other',
                  ...
                  <td class="p-5 align-middle">
                    <div class="flex gap-3 justify-center items-center text-sm">
                      <button @click="handleEdit(user.id)" class="p-1.5 rounded-lg text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition tooltip-trigger" title="Edit">
